...
def start_inference(selected_model, input_folder, store_dir, extract_instrumental, gpu_id, output_format, force_cpu, use_tta, skip_existing_files=False):
    ...
    msst_inference = multiprocessing.Process(
        ...
        args=(model_type, config_path, model_path, device, gpu_ids, output_format, use_tta, store_dict, debug, wav_bit_depth, flac_bit_depth, mp3_bit_rate, input_folder, result_queue, skip_existing_files),
        name="msst_inference"
    )
...
def run_inference(model_type, config_path, model_path, device, gpu_ids, output_format, use_tta, store_dict, debug, wav_bit_depth, flac_bit_depth, mp3_bit_rate, input_folder, result_queue, skip_existing_files=False):
    ...
    try:
        ...
        success_files = separator.process_folder(input_folder, skip_existing_files)
        separator.del_cache()
...
def run_inference_batch(model_type, config_path, model_path, device, gpu_ids, output_format, use_tta, store_dict, debug, wav_bit_depth, flac_bit_depth, mp3_bit_rate, input_folders, result_queue, skip_existing_files=False):
    ...
    批量处理多个文件夹，复用已加载的模型
    """
    logger.debug(f"Start MSST batch inference process with parameters: model_type={model_type}, config_path={config_path}, model_path={model_path}, device={device}, gpu_ids={gpu_ids}, output_format={output_format}, use_tta={use_tta}, store_dict={store_dict}, debug={debug}, wav_bit_depth={wav_bit_depth}, flac_bit_depth={flac_bit_depth}, mp3_bit_rate={mp3_bit_rate}, input_folders={input_folders}, skip_existing_files={skip_existing_files}")
    ...
    try:
        ...
        for i, input_folder in enumerate(input_folders):
            logger.info(f"处理文件夹 {i+1}/{len(input_folders)}: {input_folder}")
            try:
                success_files = separator.process_folder(input_folder, skip_existing_files)
                total_success_files.extend(success_files)
                logger.info(f"文件夹 {input_folder} 处理完成，成功处理 {len(success_files)} 个文件")
            except Exception as e:
                logger.error(f"处理文件夹 {input_folder} 失败: {str(e)}")
                # 继续处理下一个文件夹，不中断整个批处理
