    ...
    def msst_infer(self, model_type, config_path, model_path, input_folder, store_dict, output_format="wav", skip_existing_files=True):
        ...
        msst_inference = multiprocessing.Process(
            ...
            args=(
                ...
                self.mp3_bit_rate, input_folder, result_queue, skip_existing_files
            ),
    ...
    def msst_infer_batch(self, model_type, config_path, model_path, input_folders, store_dict, output_format="wav", skip_existing_files=True):
        ...
        msst_inference = multiprocessing.Process(
            ...
            args=(
                model_type, config_path, model_path, self.device, self.gpu_ids, output_format,
                self.use_tta, store_dict, self.debug, self.wav_bit_depth, self.flac_bit_depth,
                self.mp3_bit_rate, input_folders, result_queue, skip_existing_files
            ),
...
def preset_inference(input_folder, store_dir, preset_name, force_cpu, output_format, use_tta, is_audio=False, skip_existing_files=True):
    ...
    logger.debug(f"total_steps: {preset.total_steps}, force_cpu: {force_cpu}, use_tta: {use_tta}, store_dir: {store_dir}, output_format: {output_format}")
