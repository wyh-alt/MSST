...
def delete_mission(req: gr.Request, mission):
    ...
    terminated_related_dirs = set()
    
    # 检查running列表中的任务
    for running_mission in manager.running[:]:  # 使用副本避免修改迭代中的列表
        running_dirs = set()
        if getattr(running_mission, 'mission_dir', None):
            running_dirs.add(str(running_mission.mission_dir))
        batch_dirs = getattr(running_mission, 'batch_mission_dirs', None)
        if batch_dirs:
            running_dirs.update([str(d) for d in batch_dirs if d])

        if str(mission_dir) in running_dirs:
            # 找到正在运行的任务，强制终止
            ...
            terminated_related_dirs = running_dirs
            for related_dir in terminated_related_dirs:
                try:
                    if not related_dir or not os.path.isdir(related_dir):
                        continue

                    progress_path = os.path.join(related_dir, "progress.json")
                    if os.path.exists(progress_path):
                        try:
                            with open(progress_path, "r", encoding="utf-8") as f:
                                progress_data = json.load(f)
                            progress_data["status"] = "terminated"
                            ...
                            print(f"更新progress.json状态时出错: {e}")

                    mission_json_path = os.path.join(related_dir, "mission.json")
                    if os.path.exists(mission_json_path):
                        try:
                            with open(mission_json_path, "r", encoding="utf-8") as f:
                                mission_data = json.load(f)
                            mission_data["state"] = "terminated"
                            with open(mission_json_path, "w", encoding="utf-8") as f:
                                json.dump(mission_data, f, indent=4, ensure_ascii=False)
                        except Exception as e:
                            print(f"更新mission.json状态时出错: {e}")
                except Exception as e:
                    print(f"更新关联任务状态时出错: {e}")
...
def select_mission(req: gr.Request, mission):
    ...
    if not progress_info:
        ...
        if output_file_count > 0:
            try:
                end_time = os.path.getmtime(mission_dir / 'mission.json')
            except Exception:
                end_time = time.time()
...
def refresh_mission_status(req: gr.Request, mission):
    ...
    if mission_dir.exists():
        ...
        if is_running:
            progress_info = task_progress.get_progress(str(mission_dir)) or {}
            update_data = {}
            if progress_info.get('status') != 'running':
                update_data['status'] = 'running'
            if (progress_info.get('processing_start_time', 0) or 0) <= 0:
                try:
                    update_data['processing_start_time'] = os.path.getmtime(mission_json) if mission_json.exists() else time.time()
                except Exception:
                    update_data['processing_start_time'] = time.time()
            if update_data:
                task_progress.update_progress(str(mission_dir), update_data)
