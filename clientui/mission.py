...
class Manager:
    ...
    def _start_single_if_available(self):
        ...
        try:
            ...
            first.state = 'running'
            now = time.time()
            if first.mission_dir:
                task_progress.update_progress(first.mission_dir, {
                    'status': 'running',
    ...
    def _start_batch_if_available(self):
        ...
        try:
            ...
            now = time.time()
            for mission_dir in (batch_mission.batch_mission_dirs or []):
                if mission_dir and os.path.isdir(mission_dir):
                    task_progress.update_progress(mission_dir, {
                        'status': 'running',
                        'processing_start_time': now
                    })
            batch_mission.write()
