...
class TaskProgress:
    ...
    def get_total_duration(self, mission_dir):
        ...
        try:
            ...
            terminal_statuses = {'completed', 'stopped', 'terminated', 'failed'}

            if status in terminal_statuses:
                if end_time <= 0:
                    mission_json = os.path.join(mission_dir, 'mission.json')
                    try:
                        end_time = os.path.getmtime(mission_json) if os.path.exists(mission_json) else 0
                    except Exception:
                        end_time = 0
                    end_time = end_time or progress_data.get('last_update', 0) or time.time()
    ...
    def get_processing_duration(self, mission_dir):
        ...
        try:
            ...
            cached_duration = progress_data.get('processing_duration', 0) or 0
            if status in terminal_statuses and cached_duration > 0:
                return float(cached_duration)

            processing_start = progress_data.get('processing_start_time', 0) or 0
            if processing_start <= 0:
                processing_start = progress_data.get('start_time', 0) or 0
            ...
            if status in terminal_statuses:
                ...
                duration = max(0.0, end_time - processing_start) if processing_start > 0 else 0.0

                if duration <= 0:
                    log_duration = self.get_processing_duration_from_logs(mission_dir)
                    if log_duration > 0:
                        duration = float(log_duration)

                if duration > 0:
                    progress_data['processing_duration'] = duration
                    self._save_progress_directly(mission_dir, progress_data)
            ...
            if status == 'running':
                if processing_start <= 0:
                    return 0
                return max(0.0, time.time() - processing_start)
