    ...
    def process_folder(self, input_folder, skip_existing_files=False):
        ...
        for file_path in all_audio_files:
            ...
            if skip_existing_files:
                for stem in self.output_dir.keys():
                    store_dir = self.output_dir.get(stem, "")
                    if store_dir and type(store_dir) == str:
                        output_file = os.path.join(store_dir, f"{base_name}_{stem}.{self.output_format}")
                        if not os.path.exists(output_file):
                            all_outputs_exist = False
                            missing_outputs.append(f"{stem}")
                    elif store_dir and type(store_dir) == list:
                        for dir in store_dir:
                            output_file = os.path.join(dir, f"{base_name}_{stem}.{self.output_format}")
                            if not os.path.exists(output_file):
                                all_outputs_exist = False
                                missing_outputs.append(f"{stem}")
                ...
                if all_outputs_exist:
                    self.logger.info(f"跳过已存在的文件: {os.path.basename(file_path)} (所有输出文件已存在)")
                    skipped_files.append(os.path.basename(file_path))
                    continue
                else:
                    self.logger.debug(f"处理文件: {os.path.basename(file_path)} (缺少输出: {', '.join(missing_outputs)})")
